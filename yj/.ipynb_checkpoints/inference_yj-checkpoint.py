...
def inference(path, model, **kwargs):
    ...
    model.eval()
    inference_csv_path = "ds2_encoder5_inference2.csv"
    df = pd.DataFrame(columns = ['audio_path', 'Inference_result'])
    for audiofile in os.listdir(path):
        new_data = {'audio_path':audiofile, 'Inference_result':single_infer(model, os.path.join(path, audiofile))[0]}
        # DataFrame에 데이터 추가
...
#     wandb.config.update(config)
    ...
    #model load
    #load the state_dict using model.load_state_dict(torch.load(PATH), strict=False)
    state = torch.load(os.path.join(Model_PATH, '42.pt'))
    #new_state_model = {key.replace("module.", ""): value for key, value in state['model'].items()}
    model.load_state_dict(state['model'])
    if 'optimizer' in state:
        print("there is optimizer")
        #new_state_optimizer = {key.replace("module.", ""): value for key, value in state['optimizer'].items()}
        orig_optimizer.load_state_dict(state['optimizer'])
    print('Model loaded')
    
    #inference 하기
    inference(DATASET_PATH, model)
    print("Inference DONE")
#     # Assuming 'model' is an instance of torch.nn.DataParallel
#     state = torch.load(os.path.join(Model_PATH, '42.pt'))
