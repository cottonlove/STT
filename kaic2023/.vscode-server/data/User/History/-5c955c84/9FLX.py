...
class ConformerModel(nn.Module):
    def __init__(self, model_name, num_labels):
        ...
        self.convolution = nn.Conv1d(in_channels=self.bert.config.hidden_size,
                                     out_channels=128,
                                     kernel_size=3,
                                     padding=1)
    ...
    def forward(self, input_ids, attention_mask):
        ...
        last_hidden_state = last_hidden_state.transpose(1, 2)  # Shape: [batch_size, hidden_size, sequence_length]
        conv_output = self.convolution(last_hidden_state)
        conv_output = conv_output.max(dim=2)[0]  # Max pooling
