...
def sample_inference(paths, transcripts, model, **kwargs):
    recover = False
    if model.training:
        recover = True
    model.eval()
...
if __name__ == '__main__':
    ...
    if config.mode == 'train':
        ...
        sample_audio_paths = train_dataset.audio_paths[:10]
        sample_labels = train_dataset.transcripts[:10]
        print(sample_audio_paths)
        print(sample_labels)
        for epoch in range(num_epochs):
            ...
            sample_audio_paths = train_dataset.audio_paths[:10]
            sample_labels = train_dataset.transcripts[:10]
            sample_result = sample_inference(sample_audio_paths,sample_labels, model)
            for x in sample_result:
                print(f'Filename: ',x['filename'],' / Transcript: ', x['transcript'],' / Inference: ',x['text'])
