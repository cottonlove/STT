...
from modules.data import split_and_cross_validate, collate_fn
...
if __name__ == '__main__':
    ...
    args.add_argument('--seed', type=int, default=77)
    ...
    args.add_argument('--peak_lr', type=float, default=2e-05)
    ...
    if config.mode == 'train':
        ...
        lr_scheduler = get_lr_scheduler(config, optimizer, len(train_datasets[0])*5)
        optimizer = Optimizer(optimizer, lr_scheduler, int(len(train_datasets[0])*5*config.num_epochs), config.max_grad_norm)
